...
import { RetrieveAllMailUseCase } from '../use-cases/retrieve-all.use-case'

export const retrieveAllMailController: IController = async (controllerInput: IControllerInput) => {
  let session
  try {
    ...
    const response = await RetrieveAllMailUseCase.handle(
